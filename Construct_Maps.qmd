...
shift_akhi <- function(map) {
  ...
  #shift AK and HI to the right
  AKHI_SHIFT = c(250000, 0)
  akhi = map |> filter(abbr %in% c("AK", "HI"))
  
  map = map |> filter(!abbr %in% c("AK", "HI"))
  
  st_geometry(akhi) = st_geometry(akhi) + AKHI_SHIFT
  st_crs(akhi) <- st_crs(map)
  map = rbind(map, akhi)
}
```
...
#construct_state_map
```{r}
construct_state_map <- function(large_pr = F) {
  state_map <- states(resolution = "20m", cb = TRUE, year = 2020) |>
  rename(fips = GEOID,
         abbr = STUSPS,
         name = NAME) |>
  ...
  if(!large_pr){ #by default, tigris::shift_geometry scales up PR, but this isn't necessary if there are no subdivisions
  ...
  } #large_pr = T is used to provide state lines for non-state-level maps
