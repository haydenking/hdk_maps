...
```{r}
# state_key <- states(cb = TRUE, year = 2020) |>
#   st_drop_geometry() |>
#     rename(fips = GEOID,
#            abbr = STUSPS,
#            name = NAME) |>
...
# 
# write_csv(state_key, "state_key.csv")
```
These first 3 functions are necessary for dealing with AK, HI, and PR and must be loaded in for some other functions.
...
#shift_pr
```{r}
shift_pr <- function(map, caribbean = F) {
  #caribbean position is in the gulf of mexico, default is off the coast of georgia
  if(caribbean){
    PUERTO_RICO_SHIFT = c(0, 200000)
  } else {
...
#construct_state_map
```{r}
construct_state_map <- function(state_maps = T, canada = F) {
  ...
  shifted_pr = shift_pr(state_map, canada) #put PR in caribbean for Canada map
  ...
  if(canada){
    #https://www12.statcan.gc.ca/census-recensement/alternative_alternatif.cfm?l=eng&dispext=zip&teng=lpr_000b21a_e.zip&k=%20%20%20130596&loc=//www12.statcan.gc.ca/census-recensement/2021/geo/sip-pis/boundary-limites/files-fichiers/lpr_000b21a_e.zip
    canada = st_read("shapefiles/canadian_provinces/lpr_000b16a_e.shp")
    arctic <- canada |>
      filter(PREABBR %in% c("Nvt.", "N.W.T.")) |>
      st_cast("POLYGON") |>
      mutate(area = st_area(geometry)) |>
      group_by(PREABBR) |>
      slice_max(area, n = 1) |> #keep just the mainland, remove arctic islands
      ungroup()
    canada <- canada |>
      filter(!PREABBR %in% c("Nvt.", "N.W.T.")) |>
      bind_rows(arctic)
    canada <- st_transform(canada, st_crs(state_map))
    canada <- st_simplify(canada, dTolerance = if_else(state_maps, 4000, 1000), preserveTopology = F)
    canada_crosswalk <- read.csv(text = "
    PRUID,name,fips,abbr
    10,Newfoundland and Labrador,63,NL
    11,Prince Edward Island,64,PE
    12,Nova Scotia,65,NS
    13,New Brunswick,66,NB
    24,Quebec,67,QC
    35,Ontario,68,ON
    46,Manitoba,69,MB
    47,Saskatchewan,70,SK
    48,Alberta,71,AB
    59,British Columbia,59,BC
    60,Yukon,60,YT
    61,Northwest Territories,61,NT
    62,Nunavut,62,NU
    ") |>
      mutate(PRUID = as.character(PRUID),
             fips = as.character(fips))
    canada = canada |>
      select(PRUID, geometry) |>
      left_join(canada_crosswalk) |>
      select(-PRUID)
    state_map = rbind(state_map, canada)
    #currently no areas or pops for these and mapping function doesn't handle alignment well - WIP
  }
  ...
  adjustments <- tibble(
    abbr = c("CA", "NJ", "MI", "MD", "LA", "FL", "HI", "ID", "WV", "ME", "VA", "NL"), #adjust centroids
    x_adj = c(-40000, 10000,80000, -25000, -0,80000, 130000, 0, -20000, 25000, 30000, -150000),
    y_adj = c(0, 0,-120000, 35000, -40000,0,-100000, -50000, 0, -10000, 0, 0)
  )
  ...
  state_pops = read_csv("https://raw.githubusercontent.com/JoshData/historical-state-population-csv/refs/heads/main/historical_state_population_by_year.csv")
  #This source pulls FRED which pulls from the Census but FRED omits PR so I downloaded it
  
  pr_pops = read_csv("pr_pop.csv")
  #https://www.census.gov/data/tables/time-series/demo/popest/2020s-total-puerto-rico.html
  #PR population 2010-2025, used July 1 estimate for census years
  
  colnames(state_pops) = c("abbr", "year", "pop")
  state_pops = rbind(state_pops, pr_pops)
  ...
  if(canada){
  #pulled canadian populations from then cleaned manually in excel https://www150.statcan.gc.ca/t1/tbl1/en/cv.action?pid=1710000501
  canada_pops = read.csv("canada_pop00-25.csv") |>
    mutate(pop = as.numeric(str_remove_all(pop, ",")),
      pop_name = paste0("pop",sprintf("%02d", year-2000))) |>
    select(-year) |>
    pivot_wider(names_from = pop_name, values_from = pop) |>
    left_join(canada_crosswalk |> select(-PRUID, -fips), by = "name") |>
    select(-name)
    
  state_pops = canada_pops |>
    bind_rows(state_pops)
  }
  
  state_map = left_join(state_map, state_pops, by = "abbr") |>
    mutate(pop = pop20) #most reliable for US
...
shorten_puma_name <- function(name) {
  ...
  name <- sub(" PUMA", "", name)
has_amp_before_county <- grepl(
  "&.*\\b[A-Za-z]+ County\\s*\\([^)]*\\)", name
) | grepl(
  "\\b[A-Za-z]+ County\\s*\\([^)]*\\).*&", name
)

if (!has_amp_before_county) {
  name <- sub(
    "(\\b[A-Za-z]+ County\\s*\\([^)]*\\))(?!\\s*--)",
    "\\1 --",
    name,
...
} #in at least one case (Broward County (West)), the -- is missing, so I added this, but in another there is just an & so I made an exception
  parts <- strsplit(name, "--")[[1]]
  parts <- trimws(parts)
  parts <- parts[parts != ""]  # drop any empty fragments
  
  if (length(parts) == 0) return(NA_character_)
  
  # If the first part contains "City" or "Cities", keep only that
  if (grepl("\\bCity|\\bCities", parts[1], ignore.case = TRUE)) {
    base <- parts[1]
  } else {
    # drop regional commission prefix
    if (grepl("Regional Commission", parts[1], ignore.case = TRUE) & length(parts) > 1) {
      parts <- parts[-1]
    }
    if (length(parts) == 0) return(NA_character_)
  ...
  # Combine both sides if short enough
  if (length(parts) >= 2) {
    combined <- paste(parts, collapse = " - ")
    if (nchar(combined) <= 50) {
      base <- combined
    }
  }
  
  # Keep City for these known names
  keep_city <- c("Iowa City", "Sioux City", "Rapid City",
                 "Salt Lake City", "Kansas City", "Oklahoma City",
                 "New York City", "Atlantic City", "Panama City",
                 "Carson City", "Daly City", "Jersey City", "Dodge City",
                 "Garden City", "Cathedral City", "Pearl City", "Union City",
                 "Ellicott City", "Elk City", "Florida City") #might be missing a few but I think I got almost all of them
  keep_city_pattern <- paste(keep_city, collapse = "|")
  
  # Remove "City of" prefix
  base <- sub("^City of\\s+", "", base, ignore.case = TRUE)
  
  # Remove "City", "Cities", "Town" unless followed by County/Parish/Borough
  # or it's one of our known city names
  if (!is.na(base) && base != "" && !grepl(keep_city_pattern, base, ignore.case = TRUE)) {
    base <- gsub("\\b( Cities| City| Town| CDP)\\b(?!\\s*(County|Parish|Borough))", "",
                 base, perl = TRUE, ignore.case = TRUE)
  }
  ...
  base <- gsub("\\s+,", ",", base)
  base <- gsub("\\s+&", " &", base)
  base <- gsub("^,|^-|^&", "", base) #clean up beginning of name
  
  trimws(base)
}


#tigris seems to only let you get one state at a time for PUMAs so I get the shapes this way
...
construct_puma_map <- function(){
  ...
  puma_map <- puma_map |>
    ...
    rename(fips = GEOID20,
           abbr = STUSPS20,
           full_name = NAMELSAD20) |>
    filter(!abbr %in% c("GU", "MP", "VI", "AS")) |>
    mutate(
      puma = sapply(full_name, shorten_puma_name),
  ...
  puma_map <- puma_map |>
    select(puma, full_name, name, abbr, fips)
...
School Districts
```{r}
#it works but with two problems: it is a bit too HD and it looks weird with state boundaries (maybe I can get a more HD state boundary shp)
#there isn't SD-specific logic in map() (it think they're PUMAs, which is sort of ok for now)
#overlapping districts because not all are unified - is there some way to merge?
construct_sd_map <- function(){
  state_key = read_csv("state_key.csv")
  
  #it came with the (1) lol
  #https://nces.ed.gov/programs/edge/Geographic/DistrictBoundaries
  sd_map = st_read("shapefiles/EDGE_SCHOOLDISTRICT_TL25_SY2425 (1)/EDGE_SCHOOLDISTRICT_TL25_SY2425.shp") |>
    mutate(type = case_when(!is.na(ELSDLEA) ~ "elementary",
                          !is.na(SCSDLEA) ~ "secondary",
                          !is.na(UNSDLEA) ~ "unified",
                          !is.na(SDADMLEA) ~ "supervisory")) |>
    rename(name = NAME,
           fips = GEOID) |>
    filter(!STATEFP %in% c(60, 66, 69, 78)) |> #remove territories besides PR
    left_join(state_key |> select(-name), by = join_by(STATEFP == fips))
  
  #I am concerned about "School District Not Specified" in Wyoming
  
  sd_map = st_make_valid(sd_map) #sf crashes finding intersections because the geometries are weird or something but you can do this
  
  elem_sf <- sd_map |> filter(type == "elementary")
  sec_sf  <- sd_map |> filter(type == "secondary")
  uni_sf  <- sd_map |> filter(type == "unified")
  
  # pairs <- st_intersection(
  #   elem_sf |> select(elem_id = ELSDLEA),
  #   sec_sf  |> select(sec_id  = SCSDLEA)
  # ) |>
  ...
  pair_crosswalk <- pairs |>
    mutate(
      map_id = paste0("P_", elem_id, "_", sec_id),
      uni_id = NA_character_,
      paired = "paired"
    )
  
  # Elementary: use its own ID unless paired
  elem_sf <- elem_sf |>
    left_join(pair_crosswalk, by = join_by("ELSDLEA" == "elem_id")) |>
    mutate(pair_id = if_else(!is.na(map_id), map_id, paste0("E_", ELSDLEA)))
  
  # Secondary: same idea
  sec_sf <- sec_sf |>
    left_join(pair_crosswalk, by = join_by("SCSDLEA" == "sec_id")) |>
    mutate(pair_id = if_else(!is.na(map_id), map_id, paste0("S_", SCSDLEA)))
  
  uni_sf <- uni_sf |>
    mutate(pair_id = paste0("U_", UNSDLEA))
  
  sd_map <- bind_rows(elem_sf, sec_sf, uni_sf)
  
  sd_map = shift_akhi(sd_map)
  
  shifted_pr = shift_pr(sd_map)
  
  sd_map = shifted_pr$map
  puertorico = shifted_pr$puertorico
  
  puertorico$geometry = scale_sfc(puertorico$geometry, st_coordinates(st_centroid(puertorico)), 0.5)
  st_crs(puertorico) <- st_crs(sd_map)
  
  sd_map = rbind(sd_map, puertorico)

  # sd_map <- sd_map |>
  #   select(type, name, abbr, fips , LOGRADE, HIGRADE)
...
```{r}
  ...
  # na_map = construct_state_map(canada = T)
  # st_write(na_map, "shapefiles/na_map/na_map.shp", delete_dsn = T)
  na_map = st_read("shapefiles/na_map/na_map.shp")
  # still maybe WIP, might be going too hard on st_simplify, need to work on making resolution look consistent between USA and CA
  
  # sd_map = construct_sd_map()
  # st_write(sd_map, "shapefiles/sd_map/sd_map.shp", delete_dsn = T)
  sd_map = st_read("shapefiles/sd_map/sd_map.shp")
