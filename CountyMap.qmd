...
```{r}
#to do: test for planningregions = T, F

construct_county_map <- function(planningregions = F, va_merge = F, ag_districts = F) {
...
if(va_merge){ #This merge is used in Bureau of Economic Analysis data
  #va_merges was constructed from a BEA dataset
...
va_unmerged = county_map |>
  ...
  st_drop_geometry()
#there is an issue here with James City becoming James and two Fairfaxes but those won't negatively impact the final product
...
county_map = county_map |>
  filter(!fips %in% va_merged_geoms$fips) |>
...
```{r}
#For saving normal county_map (CT historical counties)
...
#for saving county_map_va_merged (Bureau of Economic Analysis data)
