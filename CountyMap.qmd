...
pr_state <- states("PR", resolution = "20m") #might need better resolution once I figure out how to scale these up

pr_municipios <- st_transform(pr_municipios, 4326)
pr_municipios <- pr_municipios |>
  mutate(county = NAMELSADCO,
         name = paste0(county, ", PR"),
         fips = paste0("72", COUNTYFP),
         abbr = "PR",
         state = "Puerto Rico",
         sqmi = ALAND / 2589988.110336, #convert from square meters to square miles
         pop = NA,
         pop10 = NA,
         pop11 = NA,
         pop12 = NA,
         ...
         pop22 = NA,
         pop23 = NA) |>
  select(-ALAND, -COUNTYFP, -NAMELSADCO)



usa = rbind(county_map_planningregions |> st_transform(4326), pr_municipios)


usa_shifted = usa

pr_idx <- usa_shifted$abbr == "PR"
#I'd like to scale up PR but everything I've tried distorts the shape terribly

st_geometry(usa_shifted)[pr_idx] <- st_geometry(usa_shifted)[pr_idx] + c(-11, 13)

usa_shifted <- st_transform(usa_shifted, usmap_crs())
# 
# ggplot(usa_shifted) +
#   geom_sf(fill = "lightblue", color = "black") +
#   theme_minimal() +
#   labs(title = "USA with Puerto Rico Shifted")
# #ADD PR to state_map
```
...
if(pr){
  ...
  st_geometry(pr_municipios) <- st_geometry(pr_municipios) + c(-11, 13) #Shift PR to be off the coast of Georgia
