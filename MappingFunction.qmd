...
#currently there needs to be a Value column or the code crashes, but adding all 1s or whatever prevents crash with no side effects
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, dollar, unit, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, increment) {
  ...
  if (!show_endpoints) {
    ...
    middle_labels <- paste0(dollar, sapply(lower_breaks[-1] + increment, shorten), " to ", 
      ...
      formatted_labels[length(formatted_labels)] <- paste0(">", dollar, shorten(lower_breaks[length(lower_breaks)]), unit)
