...
#create_inset
```{r}
#don't put top 10 if there are less than ten observations
...
#when there are 5 choropleth breaks, it is all in one line and goes onto the inset
add_theme <- function(plot, title, palette, subtitle, font_size, caption, inset_text, breaks, bubbles, max_bubble_size, continental, tag) {
  ...
  text_width <- grobWidth(textGrob(inset_text, gp = gpar(fontsize = inset_font_size))) * 1.05
  ...
  plot <- plot +
    ...
    annotation_custom(
    grob = grobTree(gList(
    #   roundrectGrob( # Shadow layer
    # x = x_pos + unit(0.005, "npc"),
    # y = y_pos - unit(0.005, "npc"),
    # width = text_width,
    # height = text_height,
    # r = unit(0.02, "npc"),
    # just = c("left", "bottom"),
    # gp = gpar(fill = alpha("black", 0.2), col = NA)  # Semi-transparent shadow
    #   ),
      roundrectGrob(
        ...
        r = unit(0.06, "npc"),
        just = c("left", "bottom"),
        gp = gpar(fill = midpoint_rgb(palette$mid_color, palette$high_color, 0.7), col = "black", lwd = 2)
      ),
    textGrob(
      inset_text,
      x = x_pos,
...
# This is for finding good bubble sizes for the legend
