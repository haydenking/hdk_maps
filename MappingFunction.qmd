...
#divergent continuous color scale
```
...
#considering removing this, I could just handle before passing the data into the mapping function
  ...
  plot <- add_theme(plot, title, palette, subtitle, caption, inset_text, breaks, bubbles, max_bubble_size, continental, tag, level, inset_box, v_adj)
...
add_theme <- function(plot, title, palette, subtitle, caption, inset_text, breaks, bubbles, max_bubble_size, continental, tag, level, inset_box, v_adj) {
  ...
  font_size = min(36*34/nchar(title), 36) #the plot is ~36 characters wide at font size 36 (Noto Sans, Arial is 40), but at font size 36 I'd still want some breathing room
  ...
  plot <- plot +
    ...
    theme(
      ...
      legend.text = element_text(size = 14, family = "notosans", color = palette$text_color, face = "bold"),
...
attempt at adding rivers and roads to city insets
```{r}
# us_roads <- primary_roads(year = 2023) %>% st_transform(3857)

# library(rnaturalearth)
# 
# # Major rivers, 10m resolution
...
#   select(geometry)
# 
# us_water = rbind(rivers10, lakes10)
  ...
  if (data_type == "Type") {
    ...
    if (is.na(breaks[1])) {
      ...
      if (!is.null(palette$cs_palette)) {
        cs_type <- if (!is.na(scale_center)) "diverging" else "sequential"
      
        cont_colors <- get_cs_palette(
          n     = n_continuous,
          type  = cs_type,
          name  = palette$cs_palette
        )
      } else {
    ...
    # Binned scale
      ...
      if (!is.na(scale_center)) {
        if(is.na(breaks[1])) {
          stop("compute_breaks doesn't work with centered scale yet. Provide breaks.")
        } 
...
create_map_inset <- function(data_3857, lon, lat, zoom_km, title) {
  ...
  # water_crop <- st_intersection(us_water, crop_box |> st_as_sfc())
  # roads_crop <- st_intersection(us_roads, crop_box |> st_as_sfc())
  
  ggplot(data = data_3857) +
    geom_sf(aes(fill = Type), linewidth = 0.25, color = "white") + #very thin PUMA borders
    # geom_sf(data = water_crop, color = "#80b1d3", size = 1.5) +
    # geom_sf(data = roads_crop, color = "gray20", size = 0.4, linetype = "solid") +
    scale_fill_manual(
        values = bin_palette,
        na.value = palette$na_color,
        na.translate = TRUE,
