...
#considering removing this, I could just handle before passing the data into the mapping function
  ...
  if (bubbles) {
    ...
    choropleth <- choropleth_map(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, numeric_type, format_value, format_value_no_unit, format_value_label, city_insets, graphicsparams, geo, rounding)
...
#color functions
```{r}
get_cs_palette <- function(n, type, name, reverse = TRUE) { #I prefer dark = high, light = low so I reverse
...
make_caption <- function(source, years = NA, caption_manual = "") {
  ...
  base_caption <- source_labels[[source]] %||% if (!is.null(year_range)) paste0("Year",s,": ", year_range) else ""
...
#choropleth_map
```{r}
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, numeric_type, format_value, format_value_no_unit, format_value_label, city_insets, graphicsparams, geo, rounding) {
  ...
  if (data_type == "Type") {
    ...
    if (is.na(breaks[1])) {
      ...
      if (!is.null(palette$cs_palette)) {
        ...
        cont_colors <- get_cs_palette(n = n_continuous, type = cs_type, name = palette$cs_palette)
      } else {
        if (is.na(scale_center)) {
      ...
      val_scaled <- rescale(map$Value, to = c(1, n_continuous))
    ...
    # Binned scale
      ...
      if (!is.null(palette$cs_palette)) {
        ...
        bin_palette <- get_cs_palette(n = n_bins, type = cs_type, name= palette$cs_palette)
      } else {
        if (is.na(scale_center)) {
  ...
  if (level == "freefloating") {
    ...
    if (continuous_scale) {
      plot <- plot +
        scale_fill_gradientn(colors = cont_colors, na.value = palette$na_color, labels = format_value_label) +
...
map <- map |>
  mutate(label = format_value_label(Value),
         label = case_when(Value < rounding & Value > 0 ~ paste0("<", format_value_label(rounding)),
                           Value > -1*rounding & Value < 0 ~ paste0(">", format_value_label(-1*rounding)), #this will look kind of weird but idk what else to do to avoid false 0s or very long labels with signif()
                           T ~ label),
         nchar = nchar(label)) |>
  filter(!str_detect(label, "NA")) #remove NA labels

  #add thresholds for label length below which some states don't need repel
  #HI: 2, VT: 1-2, NH: 2, MD: 1-2, NJ: 1-2
  #make a list of states which actually don't need to be adjusted
no_repel = map |>
  filter((nchar <= 2 & abbr %in% c("HI", "VT", "NH")) |
         (nchar == 1 & abbr %in% c("MD", "CT"))) |> #could add NJ but I would need to adjust its label south
