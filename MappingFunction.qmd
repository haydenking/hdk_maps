...
#choropleth_map
```{r}
#add ability to handle diverging color palette for continuous scale (!is.na(scale_center))
#there is a bug for continuous color scale where nothing is assigned to NA and it defaults to gray70
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, numeric_type, dollar, unit, format_value, format_value_no_unit, city_insets, graphicsparams, append_unit) {
  ...
  if (data_type == "Type") {
    ...
    if (is.na(breaks[1])) {
      ...
      gradient_func <- colorRampPalette(c(palette$mid_color, palette$high_color))
      num_colors <- 100
      gradient_colors <- gradient_func(num_colors)
      val_scaled <- scales::rescale(map$Value, to = c(1, num_colors))
      map$bin_color <- gradient_colors[round(val_scaled)]
  ...
  if (level == "freefloating") {
    ...
    if (continuous_scale) {
      plot <- plot +
        scale_color_gradientn(colors = seq_gradient_pal(palette$mid_color, palette$high_color)(seq(0,1,length.out=256)), breaks = pretty(range(map$Value), n = 4)) +
    ...
    if (continuous_scale) {
      plot <- plot + #I think there is a bug here when na_color isn't applied
        scale_fill_gradientn(colors = seq_gradient_pal(palette$mid_color, palette$high_color)(seq(0,1,length.out=256)),
                             labels = function(x) append_unit(dollar, if (unit == "%") "%" else "")(x)) +
