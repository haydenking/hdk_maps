...
```{r}
#maybe look into using these, especially part 3
...
create_inset <- function(map, level, style = c("top", "bottom"), n, inset_cutoff, cutoff_column, dollar, unit, logratio, scale_center, hide_values, inset_labels, obs_units) {
  ...
  if(level == "county"){
    map <- map |> #need to double check if this actually does anything
...
#deepseek code
 inset_data <- inset_data |>
  ...
  # Expand n if ties would push us beyond requested number
  last_rank_value <- inset_data$rank_value[min(n, nrow(inset_data))]
  actual_n <- sum(inset_data$rank_value <= last_rank_value)
...
if(obs_units == "") cat("Warning: obs_units in inset is blank.\n")
...
handle_per <- function(map, source, per, subtitle, scale_factor) {
  ...
  if (per %in% c("capita", "capita_total") ) {
    ...
    map <- map |>
      mutate(Value = Value / !!rlang::sym(per)) #this should handle sqmi
  }
...
#bubble_map
```{r}
bubble_map <- function(map, state_map, level, max_bubble_size, palette, borders, dollar, unit, region, scale_center, plusminus_labels, data_type) {
...
#choropleth_map
```{r}
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, dollar, unit, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, increment) {
  ...
  if (!show_endpoints) {
    first_label <- paste0("≤", dollar, shorten(upper_breaks[1]), unit)
    middle_labels <- paste0(dollar, sapply(lower_breaks[-1] + increment, shorten), " to ", 
                             dollar, sapply(upper_breaks[-1], shorten), unit)
      formatted_labels <- c(first_label, middle_labels)
      formatted_labels[length(formatted_labels)] <- paste0(if_else(increment == 0, ">", "≥"), dollar, shorten(lower_breaks[length(lower_breaks)] + increment), unit)
    
  } else {
    # If showing endpoints, use exact values without increment adjustment
    formatted_labels <- paste0(dollar, shorten(lower_breaks), " to ", 
                             dollar, shorten(upper_breaks), unit)
  }
} else {
  formatted_labels <- labels_manual
}
...
add_state_labels <- function(plot, map, palette, dollar, unit){
  ...
  abbr = c("NH", "MA", "CT", "NJ","MD", "RI", "DE", "IN", "VT"), # Indiana adjustment is just so it doesn't overlap with Illinois for long labels
  x_adj = c(-5000, 40000, -35000, 10000, 10000, 10000, 0, 0, -20000),
  y_adj = c(110000, 20000, -20000, -10000, -30000, 10000, 0, 35000, 40000)
)
...
map <- map |>
  ...
  mutate(label = sapply(Value, shorten),
         label = if_else(abs(Value) < 0.01 & Value != 0, "<0.01", label),
...
if("fips" %in% colnames(data)){
  ...
  if("Type" %in% colnames(data) & "Value" %in% colnames(data)){
    ...
    data_type = "Type"
    if(bubbles){
      stop("Bubble map cannot be created when only Type and not Value is supplied.")
    }
  } else {
  ...
  caption <- paste0(caption, caption_manual)
  ...
  if (bubbles) {
    ...
    plot <- bubble_map(map, state_map, level, max_bubble_size, palette, borders, dollar, unit, region, scale_center, plusminus_labels, data_type)
  } else{
    if(is.na(max_bubble_size) & level == "freefloating"){
    max_bubble_size = min(2.5, max(1.5,10/log(nrow(map))))
  }
    choropleth <- choropleth_map(map, state_map, level, breaks, labels_manual, palette, borders, dollar, unit, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, increment)
...
shorten <- function(value) {
  ...
  if(is.na(value)){
    ...
    return(paste0(signif(value / 1000000000, sigfigs), "b")) #signif rounds to a given number of sigfigs
  } else if (abs(value) > 999999) {
    return(paste0(signif(value / 1000000, sigfigs), "m"))
  } else if (abs(value) > 9999) {
    return(paste0(signif(value / 1000, sigfigs), "k"))
  } else if (abs(value) > 10) {
    return(comma_format()(round(value, 0)))
  } else{
    return(as.character(signif(value, sigfigs)))
  }
}

type_colors <- function(n_types){
    # if(n_types <= 8) {
    #   type_colors <- RColorBrewer::brewer.pal(n_types, "Dark2")
    # } else {
