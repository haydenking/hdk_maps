...
caption = make_caption(source, years, caption_manual)
  ...
  #THIS WILL CRASH ZIP MAP UNTIL FIXED (no abbr column)
...
set_palette <- function(user_palette) {
  default_palette <- list(
    ...
    "mid_color" = "#FFE5B4",  # #ECE4D5 #e0df9b #ffda9a
...
add_theme <- function(plot, title, palette, subtitle, font_size, caption, inset_text, breaks, bubbles, max_bubble_size, continental, tag, level, inset_box, v_adj) {
  ...
  negative_margin = -10
  relative_negative_margin = 0.013 #how far in the edge moves from the negative margin on the side (symmetrical)
  ...
  y_pos <- unit(-0.23 - continental_offset + v_adj, "npc") #bottom of panel, used for caption and inset
  
  #for right-aligned inset
  # x_pos <- unit(if_else(level == "state", 0.57, 0.50), "npc") #for non-right-aligned inset
  x_box <- unit(0.99 - relative_negative_margin, "npc") #right edge of inset box
  ...
  plot <- plot +
    ...
    theme(
      ...
      plot.margin = margin(if_else(continental, -110, -50), negative_margin, 0, negative_margin)
    ) +
  ...
  if(inset_box & inset_text != "") {
    ...
    annotation_custom(
      roundrectGrob(
        x = x_box, #for right-aligned inset
        y = y_pos,
    ...
    textGrob(
      inset_text,
      x = x_box - text_width + unit(0.01, "npc"),
      y = y_pos + unit(0.01, "npc"),
      # x = x_pos,
      # y = y_pos,
      just = c("left", "bottom"),
...
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, increment, dollar, unit, format_value, format_value_no_unit, city_insets, graphicsparams) {
  ...
  if (level == "freefloating") {
    ...
    plot = plot + 
      ...
      scale_fill_manual(
        ...
        na.translate = TRUE,
        labels = c(formatted_labels, na_label)
      ) + guides(fill = guide_legend(nrow = ceiling(length(breaks) / 3)))
    }
  ...
  if (level == "state") {
    ...
    plot = add_state_labels(plot, map, palette, dollar, unit)
  }
...
filepaths <- purrr::imap(city_sets, ~build_inset_grid(.x, map_3857, palette, graphicsparams, level, .y))
  }
...
add_state_labels <- function(plot, map, palette, dollar, unit){
  ...
  repels <- tibble(
    ...
    padding = c(1, 1, 1, 0.75, 1, 1, 1, 1, 1, 2)
  )
