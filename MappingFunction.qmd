...
#create_inset
```{r}
create_inset <- function(map, level, style = c("top", "bottom"), n, inset_cutoff, cutoff_column, dollar, unit, logratio, scale_center, hide_values, inset_labels) {
...
areas = case_when(level == "county" ~ "Counties",
  ...
  title_line =  paste0(topbottom, areas, 
...
add_theme <- function(plot, title, palette, subtitle, font_size, caption, inset_text, breaks, bubbles, max_bubble_size, continental, tag) {
  ...
  plot <- plot +
    ...
    grob = grobTree(gList(
      roundrectGrob(
        x = x_pos - unit(0.01, "npc"), #-0.01 for padding
        y = y_pos - unit(0.01, "npc"),
        width = text_width + unit(0.02, "npc"),  # Add padding
        height = text_height + unit(0.02, "npc"),  # Add padding
        r = unit(0.1, "npc"),
        just = c("left", "bottom"),
...
bubble_map <- function(map, state_map, level, max_bubble_size, palette, borders, dollar, unit, region, scale_center, plusminus_labels) {
  ...
  low_fill_color <- midpoint_rgb(palette$mid_color, palette$low_color, 0.7) #bias mix towards mid color
  high_fill_color <- midpoint_rgb(palette$mid_color, palette$high_color, 0.7)
  ...
  if(is.na(scale_center)){
    ...
    map <- map |> 
      mutate(Type = ifelse(Value >= 0, pos, neg))
  }
  ...
  if(is.na(scale_center)) {
    ...
    plot <- plot +
      scale_fill_manual(
      values = c(pos = high_fill_color, neg = low_fill_color),
      na.value = palette$na_color) +
      scale_color_manual(
        values = c(pos = palette$high_color, neg = palette$low_color), na.translate = FALSE)
  }
...
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, dollar, unit, region, scale_center, logratio, show_endpoints) {
  ...
  if (!is.na(breaks[1])) {
    if (!is.na(scale_center)) {
      ...
      breaks = breaks[breaks < max(map$Value, na.rm = TRUE)] #take out breaks above max value
      breaks = breaks[breaks > min(map$Value, na.rm = TRUE)] #take out breaks below min value
    ...
    breaks = compute_breaks(map$Value, 6) #only works for positive values, still a work in progress
  ...
  plot <- ggplot(data = map) + 
      with_shadow(
        geom_sf(aes(fill = bin), color = ifelse(borders, "white", NA), size = 0.1),
        x_offset = 3, y_offset = 3, sigma = 3, color = "grey70"
      ) + scale_fill_manual(
      values = bin_palette,
      na.value = palette$na_color,
  ...
  if(level == "state"){
    ...
    label_to_color <- tibble::tibble(bin = formatted_labels, bin_color = bin_palette)
    
    map <- map %>%
      left_join(label_to_color) %>%
      mutate(
        bin_color = replace_na(bin_color, palette$na_color),
        brightness = map_chr(bin_color,  ~ calculate_brightness(.x)),
        text_color = if_else(brightness > 0.5, palette$text_color, "white")
      )
    
  plot = add_state_labels(plot, map, palette) #dynamically set text color based on color of corresponding state (for states that don't have external label)
  }
...
add_state_labels <- function(plot, map, palette){
  ...
  abbr = c("NH", "MA", "CT", "NJ","MD", "RI", "DE"),
  x_adj = c(0, 50000, -35000, 0, -40000, 15000, 0),
  y_adj = c(90000, 0, -30000,40000, 0, 10000, 0)
)
...
map <- map |>
  ...
  select(-x_adj, -y_adj)
      ...
      plot <- plot + geom_text(data = map %>% filter(!(abbr %in% c( "DC","DE", "RI", "NJ", "HI", "VT", "CT", "MA", "NH"))), aes(x = x, y = y, label = lapply(Value, shorten), color = text_color), size = 4, fontface = "bold") +
...
###map
```{r}
#use type argument instead of bubbles, implement new mapping function for categorical variables?
...
map <- function(data, map, state_map = NULL, title, source, font_size = NA, years = NA, bubbles = TRUE, unit = "", per = "", max_bubble_size = NA, continental = FALSE, borders = FALSE, graphicspath, palette = list(), inset_cutoff = -1000000000, cutoff_column = "Value", breaks = NA, na_to_0 = FALSE, caption_manual = "", labels_manual = NA, inset_style = "top", subtitle = NA, region_fips = NA, cbsa = NA, migration_type = NA, merge_MSAs = F, scale_center = NA, scale_factor = 1, logratio = F, dpi = 300, show_endpoints = F, hide_values = F, tag = T, plusminus_labels = list("pos" = "Positive", "neg" = "Negative"), inset_labels = list("top" = "Top /n/", "bottom" = "Bottom /n/")) {
  ...
  if (inset_style == "hidden") {
    ...
    inset_text <- create_inset(map, level, inset_style, 10, inset_cutoff, cutoff_column, dollar, unit, logratio, scale_center, hide_values, inset_labels)
  } else if (inset_style == "split") {
    top <- create_inset(map, level, "top", 5, inset_cutoff, cutoff_column, dollar, unit, logratio, scale_center, hide_values, inset_labels)
    bottom <- create_inset(map, level, "bottom", 5, inset_cutoff, cutoff_column, dollar, unit, logratio, scale_center, inset_labels)
