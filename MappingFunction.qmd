...
if("fips" %in% colnames(data)){
  ...
  pr = map |> filter(subtr(fips, 1, 2) == "72")
...
create_inset <- function(map, level, style = c("top", "bottom"), n, inset_cutoff, cutoff_column, logratio, scale_center, hide_values, inset_labels, obs_units, format_value) {
  ...
  # Taking out DC because people complained
  if(level == "state"){
...
add_theme <- function(plot, title, palette, subtitle, font_size, caption, inset_text, breaks, bubbles, max_bubble_size, continental, tag, level, inset_box, v_adj) {
  ...
  if (!bubbles & level != "freefloating") {
    plot <- plot +
      theme(legend.key = element_rect(fill = NA, color = "black",  linewidth = 0.5))#changed from linewidth to size due to deprecation
  }
...
add_state_labels <- function(plot, map, palette, dollar, unit){
  ...
  abbr = c("NH", "MA", "CT", "NJ","MD", "RI", "DE", "IN", "VT", "DC"), # Indiana adjustment is just so it doesn't overlap with Illinois for long labels
  x_adj = c(-5000, 40000, -35000, 10000, 10000, 10000, 0, 0, -20000, 450000),
  y_adj = c(110000, 20000, -20000, -10000, -30000, 10000, 0, 35000, 30000, -350000)
)
    if(unit == "%"){
      format_value = append_unit(dollar, unit)
      percent = "%"
...
map <- map |>
  ...
  mutate(label = format_value(Value),
      ...
      plot <- plot + geom_text(data = map %>% filter(!(abbr %in% c( "DC","DE", "RI", "NJ", "HI", "VT", "CT", "MA", "NH", "MD", "PR"))), aes(x = x, y = y, label = label, color = text_color), size = 4, fontface = "bold") +
  ...
  geom_text_repel(data = map %>% filter(abbr %in% c("DE", "HI")),
    ...
    geom_text_repel(data = map %>% filter(abbr == "NJ"),
        ...
        geom_text_repel(data = map %>% filter(abbr == "PR"),
                           aes(x = x, y = y, label = label),
                           box.padding = 2, size = 4, fontface = "bold",color = palette$text_color, nudge_x = -100, nudge_y = -100)
