...
#considering removing this, I could just handle before passing the data into the mapping function
  ...
  if (inset_style == "hidden") {
    ...
    obs_units = case_when(obs_units != "auto" ~ obs_units,
                      ...
                      level == "puma" ~ "Public Use Microdata Areas", #Used to just have PUMAs but I have enough space to type it out now
  ...
  if (bubbles) {
    ...
    choropleth <- choropleth_map(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, increment, dollar, unit, format_value, format_value_no_unit, city_insets, graphicsparams, append_unit)
...
add_theme <- function(plot, title, palette, subtitle, caption, inset_text, breaks, bubbles, max_bubble_size, continental, tag, level, inset_box, v_adj) {
  ...
  plot <- plot +
    ...
    theme(
      ...
      legend.position.inside = c(relative_negative_margin + if_else(is.na(breaks[1]), 0.01 , 0), -0.02), #if color bar has a label at the left end it needs a little extra space
...
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, increment, dollar, unit, format_value, format_value_no_unit, city_insets, graphicsparams, append_unit) {
  
  continuous_scale <- FALSE
  
  # If "Type" variable exists, use categorical scale
  if (data_type == "Type") {
    map <- map %>% mutate(Type = as.factor(Type))
    type_levels <- levels(map$Type)
    type_colors <- type_colors(length(type_levels))
    ...
    if (is.na(breaks[1])) {
      ...
      # We'll directly use scale_fill_gradientn later
    ...
    # Binned scale
      ...
      # Compute breaks automatically if missing (but not NA)
      if (length(breaks) < 2) {
        num_data_points <- length(vals)
        num_breaks <- ifelse(num_data_points > 20, 5, 4)
        breaks <- compute_breaks(vals, num_breaks)
      ...
      if (is.na(labels_manual[1])) {
        ...
        if (!show_endpoints) { # \u2264 = ≤, \u2265 = ≥
          ...
          formatted_labels[length(formatted_labels)] <- paste0(if_else(increment == 0, ">", "\u2265"), 
                                                               format_value(lower_breaks[length(lower_breaks)] + increment))
        } else {
          formatted_labels <- paste0(format_value_no_unit(lower_breaks), " to ", format_value(upper_breaks))
        }
      } else {
