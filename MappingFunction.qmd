...
map <- function(data, map, state_map = NULL, title, source, years = NA, bubbles = T, unit = "", per = "", max_bubble_size = NA, continental = F, borders = F, palette = list(), inset_cutoff = -Inf, cutoff_column = "Value", breaks = NA, caption_manual = "", labels_manual = NA, inset_style = "top", subtitle = NA, region_fips = NA, merge_MSAs = F, scale_center = NA, scale_factor = 1, logratio = F, dpi = 300, show_endpoints = F, hide_values = F, tag = T, plusminus_labels = list(pos = "Positive", neg = "Negative"), inset_labels = list(top = "Top /n/", bottom = "Bottom /n/"), obs_units = "auto", na_label = "No Data", inset_box = T,  rounding = 0.1, numeric_type = "real", city_insets = F) {
  ...
  #this works but parts of it may be unused, will streamline later
  if (numeric_type == "integer") {
    format_value <- function(x) paste0(formatC(x, format="f", digits=0), unit)
  ...
  canada = if_else("BC" %in% state_map$abbr, T, F)
...
#considering removing this, I could just handle before passing the data into the mapping function
  ...
  if (inset_style == "hidden") {
    ...
    obs_units = case_when(obs_units != "auto" ~ obs_units,
                      level == "state" ~ if_else(canada, "Subdivisions", "States"), #wanted to use "States/Provinces" but didn't want to exclude the 3 Arctic territories
  ...
  if (bubbles) {
    ...
    choropleth <- choropleth_map(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, numeric_type, dollar, unit, format_value, format_value_no_unit, city_insets, graphicsparams, append_unit)
    plot = choropleth$plot
    breaks = choropleth$breaks
  }
  scale_type = if(is.na(scale_center)) "sequential" else "diverging" #feels like some duplicated code handling scale types, will streamline at some point
  
  plot <- add_theme(plot, title, palette, subtitle, caption, inset_text, breaks, bubbles, max_bubble_size, tag, level, inset_box, scale_type, canada) #add experimental npc argument
...
make_caption <- function(source, years = NA, caption_manual = "") {
  ...
  base_caption <- switch(source,
    "USDA"  = "Source: USDA Agriculture Census", #could remove "Agriculture Census" because I realized sometimes the data from the API is coming from the vegetable survey
...
add_theme <- function(plot, title, palette, subtitle, caption, inset_text, breaks, bubbles, max_bubble_size, tag, level, inset_box, scale_type, canada) {
...
if (names(dev.cur()) == "null device") {
  ...
  subtitle_offset = 0.07*(title_font_size/36) # Vertical adjustment for subtitle
  ...
  x_box_right <- unit(if_else(canada | level == "puma", 0.99, 0.79), "npc") #right edge of inset box
    ...
    x_box_left <- x_box_right - box_width / 2
  
# if (names(dev.cur()) == "null device") grDevices::pdf(NULL) #moved earlier
  ...
  plot <- plot +
    ...
    plot = plot +
      annotation_custom(
        textGrob(
          inset_text,
          x = x_box_right - box_width + padding, # x_box_left + padding??
          y = y_pos + padding,
          just = c("left", "bottom"),
          gp = gpar(
            fontsize = inset_font_size,
...
plot <- plot +
  draw_label_box(
    ...
    x = 0.5,
    y = title_height - subtitle_offset - if_else(is.na(subtitle), 0, subtitle_offset / 2),
    fontfamily = "lekton",
    fontsize = 12,
    text_color = palette$title_color,
    fill_color = palette$background_color,
...
#choropleth_map
```{r}
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, numeric_type, dollar, unit, format_value, format_value_no_unit, city_insets, graphicsparams, append_unit) {
  ...
  if (level == "freefloating") {
    ...
    if (continuous_scale) {
      plot <- plot +
        scale_fill_gradientn(colors = cont_colors, na.value = palette$na_color, labels = function(x) append_unit(dollar, if (unit == "%") "%" else "")(x)) +
  ...
  if (level == "state") {
    ...
    plot <- add_state_labels(plot, map, palette, dollar, unit, append_unit)
  }
...
add_state_labels <- function(plot, map, palette, dollar, unit, append_unit){
  font = "notosans"
...
map <- map |>
  mutate(label = format_value(Value),
         label = case_when(Value < 0.01 & Value > 0 ~ paste0("<", dollar, "0.01", percent), 
                           Value > -0.01 & Value < 0 ~ paste0(">-", dollar, "0.01", percent), #this will look kind of weird but idk what else to do to avoid false 0s or very long labels with signif()
                           T ~ label),
         nchar = nchar(label)) |>
...
repels <- repels |>
  filter(!abbr %in% no_repel)

adjustments <- adjustments |>
  filter(!abbr %in% no_repel)

map <- map |>
  left_join(adjustments, by = "abbr") |> #I think all the adjustment are fine, maybe VT and NH should be dropped if it doesn't get repelled
  mutate(
...
if(nrow(wv) == 1){
  ...
  if(wv_label_length > 3){ #4 characters is borderline ok
    map <- map|>
      filter(abbr != "WV")
    plot <- plot +
      geom_text(
