...
#I don't like the way this one handles values between 1 and 10, leaving an extra .0 on there when it's a whole number

# shorten <- function(x) {
#   label_number(
#     scale_cut = cut_short_scale(),
#     accuracy = case_when(abs(x) >= 10 ~ 1,
#                          abs(x) >= 1 ~ 0.1,
#                          T ~ 0.01)
#   )(x)
# }
...
shorten <- function(value) {
  ...
  abs_val <- abs(value)
    ...
    return(comma_format()(round(value)))
  }

  # Thresholds and suffixes
  thresholds <- c(1e12, 1e9, 1e6, 1e3)
  suffixes   <- c("t",   "b",  "m",  "k")

  for (i in seq_along(thresholds)) {
    if (abs_val >= thresholds[i]) {
      display_value = value / thresholds[i]
      if(display_value >= 100) sigfigs = 3 else sigfigs = 2
      return(paste0(signif(display_value, sigfigs), suffixes[i]))
    }
  }
    return(as.character(signif(value, 2)))
}
...
create_inset <- function(map, level, style = c("top", "bottom"), n, inset_cutoff, cutoff_column, dollar, unit, logratio, scale_center, hide_values, inset_labels, obs_units) {
 ...
 inset_data <- inset_data |>
  ...
  n = nrow(inset_data) #in case there are less than 10 observations (needed for some state maps) or ties in inset
...
choropleth_map <- function(map, state_map, level, breaks, labels_manual, palette, borders, dollar, unit, region, scale_center, logratio, show_endpoints, na_label, max_bubble_size, data_type, increment) {
  ...
  if (!show_endpoints) {
    first_label <- paste0(if_else(increment == 0, "<", "≤"), dollar, shorten(upper_breaks[1]), unit)
...
add_state_labels <- function(plot, map, palette, dollar, unit){
  ...
  y_adj = c(110000, 20000, -20000, -10000, -30000, 10000, 0, 35000, 30000)
)
